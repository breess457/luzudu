...
const Mymarket = ()=>{
    ...
    const {getcookie, getprofile,getmymarket,getImageMarket} = useAuth()
    const [showInputShopName, setShowInputShopName] = useState(false)
    const [showLocation, setShowLocation] = useState(false)
    const [inputShopName, setInputShopName] = useState(getmymarket?.nameMarket ?? "")
    const [inputShopLocation, setInputShopLocation] = useState({
        address:getmymarket.length > 0 ? getmymarket.locationMarket[0]?.address : "",
        subdistrict:getmymarket.length > 0 ? getmymarket.locationMarket[0]?.subdistrict : "",
        district:getmymarket.length > 0 ? getmymarket.locationMarket[0]?.district : "",
        province:getmymarket.length > 0 ? getmymarket.locationMarket[0]?.province : "",
        zipcode:getmymarket.length > 0 ? getmymarket.locationMarket[0]?.zipcode : ""
    })
    ...
    const handleSubmitShopName = async (e:FormEvent)=>{
        ...
        if(!inputShopName){
            ...
            if(!postShopName.ok) throw new Error(`Error Post Data:${postShopName.status}`)
                console.log(postShopName.status)
        ...
        <div className="w-full">
                            ...
                            <div className="w-full flex flex-row mx-7">
                                ...
                                </span>
                                {
                                    ...
                                    ? (
                                        ...
                                        getmymarket.length > 0 ? (
                                        <>
                                            <span className="text-xl font-semibold ml-4 mr-2">{ getmymarket.locationMarket[0]?.address}</span>
                                            <span className="text-xl font-semibold mx-2">{ getmymarket.locationMarket[0]?.subdistrict}</span>
                                            <span className="text-xl font-semibold mx-2">{ getmymarket.locationMarket[0]?.district}</span>
                                            <span className="text-xl font-semibold mx-2">{ getmymarket.locationMarket[0]?.province}</span>
                                            <span className="text-xl font-semibold mx-2">{ getmymarket.locationMarket[0]?.zipcode}</span>
                                        </>
                                        ):null
                                    )
                                }
            ...
            <div className="bg-white w-full max-w-full mx-auo overflow-hidden mt-4">
                <ul className="grid grid-flow-col text-center border-b border-gray-200 text-gray-500">
                  ...
                  <li>
                    <a href="#page1" className="flex justify-center border-b-4 border-transparent hover:text-indigo-600 hover:border-indigo-600 py-4">รายการสินค้าที่ขาย</a>
                  </li>
                  <li>
                    <a href="#page2" className="flex justify-center border-b-4 border-transparent hover:text-indigo-600 hover:border-indigo-600 py-4">รายการสินค้าที่ต้องจัดส่ง</a>
                  </li>
                  <li>
                    <a href="#page3" className="flex justify-center border-b-4 border-transparent hover:text-indigo-600 hover:border-indigo-600 py-4">สินค้าที่ส่งเรียบร้อย</a>
                  </li>
                  <li>
                    <a href="#page5" className="flex justify-center border-b-4 border-transparent hover:text-indigo-600 hover:border-indigo-600 py-4">สินค้าที่ถูกตีกลับ</a>
                  </li>
                </ul>
            ...
            <div className="w-full flex mt-3">
                <button className="ml-auto px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">เพิ่มสินค้า</button>
            </div>
            {modelUploadImage ? <EditImageMarket setModelUploadImage={setModelUploadImage} /> : null}
        </div>
    )
}
